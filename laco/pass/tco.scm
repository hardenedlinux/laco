...
(define* (tco expr #:optional (tail-safe? #f))
        ...
        `(,@(map tco (list-head exprs len))
          ,(match tail
             (($ app/k ($ cps _ kont _ _) _ (k _ ...))
              (=> failed!)
              (cond
               ((and tail-safe? (not (kont-eq? k prim:return)) (kont-eq? kont k))
  ...
  (match expr
         ...
         (($ app/k _ _ (k _ ...))
          (cond
           ((and (kont-eq? f kont) (kont-eq? kont k))
            ...
            (tco (car (app/k-args expr)) tail-safe?))
           (else (failed!))))
         (else (failed!)))))
    (($ app/k ($ cps _ kont _ _) f args)
     (when (and tail-safe?
                (is-tail-call? expr)
                (kont-eq? (car args) kont)
    ...
    (($ letfun/k ($ bind-special-form/k _ var value body))
     (parameterize ((current-def (id-name var)))
       (bind-special-form/k-value-set! expr (tco value tail-safe?))
       (bind-special-form/k-body-set! expr (tco body #t))
       expr))
    ((? bind-special-form/k?)
     (bind-special-form/k-value-set! expr (tco (bind-special-form/k-value expr) tail-safe?))
    ...
    (($ lambda/k _ _ (? app/k? a))
     ...
     expr)
    (($ lambda/k _ _ body)
     (lambda/k-body-set! expr (tco body tail-safe?))
     expr)
